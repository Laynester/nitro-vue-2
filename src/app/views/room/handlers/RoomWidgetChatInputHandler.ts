import { AvatarExpressionEnum, HabboClubLevelEnum, Nitro, NitroEvent, RoomControllerLevel, RoomSessionChatEvent, RoomWidgetEnum, RoomZoomEvent, TextureUtils } from '@nitrots/nitro-renderer';
        ...
        switch (message.type)
        {
            ...
            case RoomWidgetChatMessage.MESSAGE_CHAT: {
                ...
                if (parts.length > 0)
                {
                    ...
                    {
                        const selectedAvatarId = Nitro.instance.roomEngine.selectedAvatarId;
                    ...
                    switch (firstPart.toLowerCase())
                    {
                        case ':d':
                        case ';d':
                            if (Nitro.instance.sessionDataManager.clubLevel === HabboClubLevelEnum.VIP)
                            {
                        ...
                        case ':sign':
                            ...
                            return null;
                        case ':iddqd':
                            Nitro.instance.roomEngine.events.dispatchEvent(new RoomZoomEvent(this.container.roomSession.roomId, -1, true));

                            return null;
                        case ':zoom':
                            Nitro.instance.roomEngine.events.dispatchEvent(new RoomZoomEvent(this.container.roomSession.roomId, parseInt(secondPart), false));

                            return null;
                        case ':screenshot':
                            const texture = Nitro.instance.roomEngine.createTextureFromRoom(this.container.roomSession.roomId, 1);
                        ...
                        case ':billsonnn':
                            ...
                            return null;
                        case ':settings':
                            if (this.container.roomSession.isRoomOwner || Nitro.instance.sessionDataManager.isModerator)
                            {
            ...
            case RoomWidgetChatSelectAvatarMessage.MESSAGE_SELECT_AVATAR: {
                ...
                Nitro.instance.roomEngine.setSelectedAvatar(selectedEvent.roomId, selectedEvent.objectId);
                break;
            }
        }
