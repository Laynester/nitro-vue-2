...
export default {
    methods: {
        selectTab(code) {
            NavigatorListener.getInstance().setCurrentContextByCode(code);
...
<template>
    <div class="d-flex">
        ...
        >
            <div class="tab-icon guest" />
            <span>{{$filters.localizeText('navigator.toplevelview.hotel_view')}}</span>
        </div>
    </div>
</template>